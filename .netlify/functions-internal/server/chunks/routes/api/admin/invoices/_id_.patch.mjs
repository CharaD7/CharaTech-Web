import { d as defineEventHandler, r as requireAdmin, f as getRouterParam, c as createError, b as readBody, p as prisma, e as sendEmail } from '../../../../_/nitro.mjs';
...
const buildInvoiceEmail = (invoice, client) => {
  const sym = { USD: "$", EUR: "\u20AC", GBP: "\xA3", GHS: "\u20B5", CAD: "C$", AUD: "A$" };
  const cur = sym[invoice.currency] || "$";
  const fmt = (n) => `${cur}${n.toFixed(2).replace(/\B(?=(\d{3})+(?!\d))/g, ",")}`;
  ...
  const taxLine = invoice.taxAmount && invoice.taxAmount > 0 ? `<tr><td style="padding:6px 0;color:#a78bfa;">Tax / Levy</td><td style="padding:6px 0;text-align:right;color:#a78bfa;">${fmt(invoice.taxAmount)}</td></tr>` : "";
...
<body style="margin:0;padding:0;background:#0f0a1e;font-family:'Segoe UI',system-ui,sans-serif;">
  <div style="max-width:680px;margin:0 auto;padding:40px 20px;">
    ...
    <div style="background:rgba(15,10,30,0.95);border:1px solid rgba(168,85,247,0.15);border-top:none;border-bottom:none;padding:24px 40px;">
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:24px;">
        <div>
          ...
          <div style="color:#f1f5f9;font-weight:600;">${(client == null ? void 0 : client.fullName) || "Valued Client"}</div>
          <div style="color:#94a3b8;font-size:13px;">${(client == null ? void 0 : client.email) || ""}</div>
          ${(client == null ? void 0 : client.companyName) ? `<div style="color:#94a3b8;font-size:13px;">${client.companyName}</div>` : ""}
        </div>
        <div style="text-align:right;">
    ...
    <div style="background:linear-gradient(135deg,#1e0a3c,#2d0b55);border:1px solid rgba(168,85,247,0.3);border-top:none;border-radius:0 0 16px 16px;padding:32px 40px;text-align:center;">
      ...
      <p style="color:#4b5563;font-size:11px;margin:24px 0 0;">CharaTech \xB7 Software Requirements Platform \xB7 hello@charatech.com</p>
    </div>
...
const _id__patch = defineEventHandler(async (event) => {
  try {
    ...
    if (status === "SENT") {
      ...
      if (client == null ? void 0 : client.email) {
